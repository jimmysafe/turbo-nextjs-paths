...
export async function up({ payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await payload.db.drizzle.run(sql`CREATE TABLE \`stores\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`name\` text,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL
  ...
  `);
  await payload.db.drizzle.run(sql`CREATE INDEX \`stores_updated_at_idx\` ON \`stores\` (\`updated_at\`);`);
  await payload.db.drizzle.run(sql`CREATE INDEX \`stores_created_at_idx\` ON \`stores\` (\`created_at\`);`);
  ...
  await payload.db.drizzle.run(sql`CREATE INDEX \`payload_locked_documents_rels_stores_id_idx\` ON \`payload_locked_documents_rels\` (\`stores_id\`);`);
...
export async function down({ payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await payload.db.drizzle.run(sql`DROP TABLE \`stores\`;`);
